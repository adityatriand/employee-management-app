        ...
        <form method="GET" action="{{ route('assets.index') }}" id="filterForm">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6 text-end">
                    ...
                    @if($hasFilters)
                    <a href="{{ route('assets.index') }}" class="btn btn-outline-danger ms-2">
            ...
            <div class="mt-3">
                <div class="d-flex flex-wrap gap-2">
                    ...
                    <span class="badge bg-primary">
                        ...
                        <a href="{{ route('assets.index', array_merge(request()->except('search'), ['page' => 1])) }}" class="text-white ms-2" style="text-decoration: none;">×</a>
                    </span>
                    @endif
                    @if(request('asset_type'))
                    <span class="badge bg-info">
                        Tipe: {{ ucfirst(request('asset_type')) }}
                        <a href="{{ route('assets.index', array_merge(request()->except('asset_type'), ['page' => 1])) }}" class="text-white ms-2" style="text-decoration: none;">×</a>
                    </span>
                    @endif
                    @if(request('status'))
                    <span class="badge bg-warning">
                        Status: {{ ucfirst(request('status')) }}
                        <a href="{{ route('assets.index', array_merge(request()->except('status'), ['page' => 1])) }}" class="text-white ms-2" style="text-decoration: none;">×</a>
                    </span>
                    @endif
                    @if(request('assigned_to'))
                    <span class="badge bg-success">
                        Ditugaskan: {{ request('assigned_to') == '0' ? 'Tidak Ditugaskan' : ($employees->find(request('assigned_to'))->name ?? 'N/A') }}
                        <a href="{{ route('assets.index', array_merge(request()->except('assigned_to'), ['page' => 1])) }}" class="text-white ms-2" style="text-decoration: none;">×</a>
                    </span>
                    @endif
                    @if(request('department'))
                    <span class="badge bg-secondary">
                        Departemen: {{ request('department') }}
                        <a href="{{ route('assets.index', array_merge(request()->except('department'), ['page' => 1])) }}" class="text-white ms-2" style="text-decoration: none;">×</a>
                    </span>
                    @endif
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="{{ route('assets.show', $asset->id) }}" class="d-inline-block">
                        ...
                        <td>
                            @if($asset->assignedTo)
                            <a href="{{ route('employees.show', $asset->assignedTo->id) }}" class="text-decoration-none">
